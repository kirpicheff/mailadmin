    ...
    <div class="bg-white dark:bg-slate-900 rounded-[32px] w-full max-w-xl shadow-2xl border border-white/10 dark:border-slate-800/50 animate-in zoom-in duration-300 overflow-hidden">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">
              {{ isEdit ? 'Настройка алиаса' : (isDomainAlias ? 'Новый алиас домена' : 'Новый алиас') }}
            </h3>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ isEdit ? form.address : 'Перенаправление почты' }}</p>
          </div>
        </div>
      ...
      <form @submit.prevent="save" class="p-8 space-y-6">
        ...
        <div v-if="!isEdit" class="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-2xl">
          <button type="button" @click="isDomainAlias = false" :class="!isDomainAlias ? 'bg-white dark:bg-slate-700 shadow-sm text-indigo-600' : 'text-slate-500'" class="flex-1 py-2 text-xs font-black uppercase tracking-widest rounded-xl transition-all">Обычный</button>
          <button type="button" @click="isDomainAlias = true" :class="isDomainAlias ? 'bg-white dark:bg-slate-700 shadow-sm text-amber-600' : 'text-slate-500'" class="flex-1 py-2 text-xs font-black uppercase tracking-widest rounded-xl transition-all">Доменный</button>
        </div>

        <!-- Источник -->
        <div v-if="!isDomainAlias" class="relative">
          <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5 ml-1">Псевдоним (Источник)</label>
        ...
        <div v-else class="space-y-4">
          <div class="relative">
            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5 ml-1">Домен-источник (Алиас)</label>
          ...
          <div class="relative">
            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5 ml-1">Целевой домен (Куда слать почту)</label>
            <CustomSelect 
              v-model="form.target_domain" 
              :options="domains.map(d => ({ label: d.domain, value: d.domain }))"
            />
          </div>
          <p class="text-[9px] text-slate-400 ml-1 italic font-bold">Вся почта, поступающая на {{ form.alias_domain || '...' }}, будет перенаправлена на {{ form.target_domain }}</p>
        </div>

        <!-- Получатели -->
        <div v-if="!isDomainAlias" class="relative">
          <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1.5 ml-1">Куда пересылать (один или несколько адресов)</label>
          <textarea v-model="form.goto" placeholder="user1@example.com&#10;user2@other.com" required rows="4"
          ...
          <p class="text-[9px] text-slate-400 mt-2 ml-1 font-bold tracking-tight uppercase opacity-70">Можно вставить список через запятую или с новой строки</p>
        </div>
        ...
        <div class="flex items-center justify-between bg-slate-50/30 dark:bg-slate-800/10 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/50">
          <div class="flex flex-col">
            ...
            <span class="text-[10px] text-slate-400 uppercase font-black">Включить/выключить пересылку</span>
          </div>
  ...
  try {
    if (isDomainAlias.value) {
      ...
      if (isEdit.value) {
        ...
        alert('Редактирование имени доменного алиаса пока не поддерживается. Удалите и создайте новый.')
        return
      } else {
