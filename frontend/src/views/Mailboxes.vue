  ...
  <div class="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
    ...
    <div class="glass-panel p-6 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col md:flex-row items-center gap-6">
      <div class="w-full md:w-1/3 relative">
        <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Выбор домена</label>
      ...
      <div class="w-full md:w-2/3 relative group">
        <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Поиск</label>
        <div class="relative">
          ...
          <input v-model="searchQuery" type="text" placeholder="Поиск по адресу или имени..."
    ...
    <div v-if="selectedDomain && !searchQuery" class="mb-10 animate-in fade-in slide-in-from-top-4 duration-700">
      <div class="glass-panel p-10 text-center relative overflow-hidden group">
        <h2 class="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-4">Обзор для {{ selectedDomain }} :</h2>
        <div class="flex flex-col items-center gap-1 text-sm font-bold text-slate-600 dark:text-slate-400">
          <p>Алиасы: {{ stats.aliases_count }} <span class="text-slate-400">/</span> {{ stats.aliases === 0 ? 'Неограничено' : stats.aliases }}</p>
          <p>Ящики: {{ stats.mailboxes_count }} <span class="text-slate-400">/</span> {{ stats.mailboxes === 0 ? 'Неограничено' : stats.mailboxes }}</p>
        </div>
        ...
        <div class="flex flex-wrap justify-center items-center gap-2 pt-4 text-xs font-black uppercase tracking-widest border-t border-slate-100 dark:border-slate-800/50 mt-4">
          <span class="text-slate-400">Показать:</span>
          <button @click="filterType = 'all'" :class="filterType === 'all' ? 'text-mail-blue-600' : 'text-slate-400 hover:text-slate-600 dark:hover:text-white'" class="transition-colors px-2">Все</button>
          <span class="text-slate-200 dark:text-slate-800">::</span>
          <button @click="filterType = 'boxes'" :class="filterType === 'boxes' ? 'text-mail-blue-600' : 'text-slate-400 hover:text-slate-600 dark:hover:text-white'" class="transition-colors px-2">Ящики</button>
          <span class="text-slate-200 dark:text-slate-800">::</span>
          <button @click="filterType = 'aliases'" :class="filterType === 'aliases' ? 'text-mail-blue-600' : 'text-slate-400 hover:text-slate-600 dark:hover:text-white'" class="transition-colors px-2">Алиасы</button>
          <span class="text-slate-200 dark:text-slate-800">::</span>
          <button @click="filterType = 'domain_aliases'" :class="filterType === 'domain_aliases' ? 'text-mail-blue-600' : 'text-slate-400 hover:text-slate-600 dark:hover:text-white'" class="transition-colors px-2">Алиасы доменов</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="selectedDomain || searchQuery" class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl rounded-[32px] overflow-hidden">
      <div class="flex items-center justify-between px-8 py-6 border-b border-slate-100 dark:border-slate-800">
        <h2 class="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">
          {{ searchQuery ? 'Результаты поиска по всем доменам' : 'Ресурсы домена' }}
        </h2>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-700/50">
              <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Адрес / Тип</th>
              <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Куда / Инфо</th>
              <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Статус</th>
              <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Изменен</th>
              <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Действия</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 font-bold">
            ...
            <tr v-if="filteredItems.length === 0 && !loading">
              <td colspan="5" class="px-8 py-20 text-center text-slate-400 font-bold italic">
                {{ searchQuery ? 'По вашему запросу ничего не найдено' : 'В этом разделе пока пусто' }}
              </td>
            </tr>
          </tbody>
        </table>
...
const deleteItem = async (item) => {
  if (!confirm(`Удалить ${item.address}?`)) return
  try {
    const endpoint = item.type === 'mailbox' ? `/mailboxes/${item.address}` : 
                     (item.type === 'alias' ? `/aliases/${item.address}` : `/aliases/domain-aliases/${item.address}`)
