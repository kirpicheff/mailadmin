<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Настройки доступа</h1>
    ...
    <div class="glass-panel overflow-hidden">
      <table class="w-full text-left">
        <thead class="bg-slate-50 dark:bg-slate-800/50">
          <tr>
            <th class="px-6 py-3 text-xs font-medium text-slate-500 uppercase">Пользователь</th>
            <th class="px-6 py-3 text-xs font-medium text-slate-500 uppercase">Права</th>
            <th class="px-6 py-3 text-xs font-medium text-slate-500 uppercase">Активен</th>
            <th class="px-6 py-3 text-xs font-medium text-slate-500 uppercase">Почта / Телефон</th>
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-700/50">
          <tr v-for="admin in admins" :key="admin.username" class="group hover:bg-slate-50/80 dark:hover:bg-mail-blue-500/5 transition-all duration-200">
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div class="text-xs text-slate-500">{{ admin.email_other || 'Нет доп. почты' }}</div>
                </div>
              </div>
            </td>
            <td class="px-6 py-4">
            ...
            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">
              ...
              <div v-else class="text-xs opacity-40">Не указан</div>
            </td>
            <td class="px-6 py-4">
              <div class="flex items-center gap-2">
                <button @click="editAdmin(admin)" class="p-2 bg-slate-100 hover:bg-mail-blue-100 text-slate-600 hover:text-mail-blue-600 dark:bg-slate-800/50 dark:hover:bg-mail-blue-900/40 dark:text-slate-400 dark:hover:text-mail-blue-300 rounded-xl transition-all duration-200" title="Изменить">
                ...
                <button @click="deleteAdmin(admin.username)" class="p-2 bg-slate-100 hover:bg-red-100 text-slate-600 hover:text-red-600 dark:bg-slate-800/50 dark:hover:bg-red-900/40 dark:text-slate-400 dark:hover:text-red-300 rounded-xl transition-all duration-200" title="Удалить">
      ...
      <div class="bg-white dark:bg-slate-900 p-8 rounded-3xl w-full max-w-lg shadow-[0_20px_50px_rgba(0,0,0,0.3)] space-y-6 border border-white/20 dark:border-slate-800/50">
        <div class="flex justify-between items-center">
          <h3 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight">
            {{ isEdit ? 'Редактирование' : 'Новый администратор' }}
          </h3>
        ...
        <form @submit.prevent="saveAdmin" class="space-y-5">
          <div class="space-y-4">
            <!-- Username -->
            <div class="relative">
              <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 ml-1">Username (Email)</label>
              <div class="relative">
            ...
            <div class="grid grid-cols-2 gap-5">
              <div class="relative">
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 ml-1">Пароль</label>
                <div class="relative">
                  <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">
              ...
              <div class="flex flex-col justify-center gap-4">
                ...
                <label class="flex items-center justify-between gap-4 cursor-pointer group bg-slate-100 dark:bg-slate-800/50 p-2.5 px-4 rounded-2xl transition-all hover:bg-slate-200 dark:hover:bg-slate-700/50">
                  <span class="text-xs font-bold uppercase tracking-wider text-slate-500 group-hover:text-mail-blue-500 transition-colors">Суперадмин</span>
                ...
                <label class="flex items-center justify-between gap-4 cursor-pointer group bg-slate-100 dark:bg-slate-800/50 p-2.5 px-4 rounded-2xl transition-all hover:bg-slate-200 dark:hover:bg-slate-700/50">
                  <span class="text-xs font-bold uppercase tracking-wider text-slate-500 group-hover:text-green-500 transition-colors">Статус: Активен</span>
            ...
            <div class="grid grid-cols-2 gap-5">
              <div class="relative">
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 ml-1">Доп. Email</label>
                <div class="relative">
                  <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">
              ...
              <div class="relative">
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 ml-1">Телефон</label>
                <div class="relative">
                  <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">
            ...
            <div v-if="!form.superadmin" class="space-y-3 pt-2">
              <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 ml-1">Доступ к доменам</label>
          ...
          <div class="flex gap-4 pt-4">
            <button type="submit" class="flex-1 py-4 bg-mail-blue-600 text-white rounded-2xl font-bold hover:bg-mail-blue-700 shadow-lg shadow-mail-blue-500/30 transition-all active:scale-95">
              {{ isEdit ? 'Сохранить изменения' : 'Создать администратора' }}
            </button>
