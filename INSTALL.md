...
## Вариант 1. Автоматическая установка (Рекомендуемый)

Для быстрой установки и настройки автоматических обновлений выполните команду на сервере:

```bash
curl -sSL https://raw.githubusercontent.com/kirpicheff/mailadmin/main/install.sh | bash
```

Этот скрипт:
1. Клонирует репозиторий в `/opt/mailadmin`.
2. Создаст конфигурационный файл `.env` (если его нет).
3. Соберет и запустит проект через Docker Compose.
4. Настроит Cron-задачу для проверки обновлений в GitHub каждый час.
...
> Агент внутри контейнера использует команды `postqueue`, `postsuper` и `fail2ban-client` для управления системой. В стандартном `Dockerfile` эти утилиты не предустановлены для уменьшения размера образа. 
> Если вы планируете полноценно управлять очередью и банами из Docker-контейнера, вам необходимо:
> 1. Либо доустановить их в `Dockerfile` (через `apk add postfix fail2ban`).
> 2. Либо пробросить бинарники и сокеты управления с хост-машины через `volumes`.
